...
from common.task_validation import incomplete_dependencies, validate_task_graph
...
def _record_start_state(
    ...
    if not data.get("branch"):
        ...
        if branch:
            ...
            meta = data.get("meta", {})
            direct_delivery = (
                isinstance(meta, dict) and meta.get("delivery_mode") == "direct"
            )
            if branch == data.get("base_branch") and not direct_delivery:
...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    metadata_errors = validate_task_graph(full_path, repo_root)
    if metadata_errors:
        print(colored("Error: task metadata validation failed", Colors.RED))
        for error in metadata_errors:
            print(f"  - {error}")
        return 1
    blocked_dependencies = incomplete_dependencies(full_path, repo_root)
    if blocked_dependencies:
        print(colored("Error: task dependencies are not completed", Colors.RED))
...
Examples:
  ...
  python task.py add-context <dir> implement .trellis/spec/backend/error-handling.md "Error handling guidelines"
