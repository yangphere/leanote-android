...
def safe_archive_paths_to_add(
    ...
    Scoped to ONLY the paths the archive operation actually touched:
      ...
      - the freshly moved task's exact archive destination
    ...
    if task_name is not None:
        ...
        # explicitly.
        if archive_destination is not None and archive_destination.is_dir():
            try:
                paths.append(
                    archive_destination.resolve().relative_to(repo_root.resolve()).as_posix()
                )
            except ValueError:
                return []
        elif archive_dir.is_dir():
            # Compatibility for older callers. New archive callers pass the
            # exact destination so unrelated archived tasks stay unstaged.
            paths.append(f"{DIR_WORKFLOW}/{DIR_TASKS}/{DIR_ARCHIVE}")
