...
from .task_validation import validate_task_graph
...
def cmd_validate(args: argparse.Namespace) -> int:
    ...
    total_errors = 0
    metadata_errors = validate_task_graph(target_dir, repo_root)
    for error in metadata_errors:
        print(f"  {colored('task.json: ' + error, Colors.RED)}")
    total_errors += len(metadata_errors)
