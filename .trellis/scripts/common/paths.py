...
_DEVELOPER_NAME_RE = re.compile(r"^[A-Za-z0-9](?:[A-Za-z0-9_-]{0,63})$")
...
def is_valid_developer_name(name: object) -> bool:
    """Whether a developer identity is a path-safe portable slug."""
    return isinstance(name, str) and _DEVELOPER_NAME_RE.fullmatch(name) is not None
...
def _read_developer_file(dev_file: Path) -> str | None:
    ...
    for line in content.splitlines():
        if line.startswith("name="):
            ...
            return name if is_valid_developer_name(name) else None
...
def get_developer(repo_root: Path | None = None) -> str | None:
    ...
    if is_valid_developer_name(env_name):
        return env_name
