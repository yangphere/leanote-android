...
def init_developer(name: str, repo_root: Path | None = None) -> bool:
    ...
    """
    if not is_valid_developer_name(name):
        print(
            "Error: developer name must be a 1-64 character ASCII slug "
            "using only letters, numbers, underscores, and hyphens",
            file=sys.stderr,
        )
        return False
...
"""
    ...
    # Publish the identity only after its workspace is complete.
    initialized_at = datetime.now().isoformat()
    temp_dev_file = dev_file.with_name(f"{dev_file.name}.tmp")
    try:
        temp_dev_file.write_text(
            f"name={name}\ninitialized_at={initialized_at}\n", encoding="utf-8"
        )
        temp_dev_file.replace(dev_file)
    except (OSError, IOError) as e:
        try:
            temp_dev_file.unlink(missing_ok=True)
        except OSError:
            pass
