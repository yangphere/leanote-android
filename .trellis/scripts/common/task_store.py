...
def _validate_branch_metadata(
    ...
    meta = data.get("meta", {})
    direct_delivery = isinstance(meta, dict) and meta.get("delivery_mode") == "direct"
    ...
    if branch and base_branch and branch == base_branch and not direct_delivery:
        print(
            colored(
...
def cmd_archive(args: argparse.Namespace) -> int:
    ...
    metadata_errors = validate_task_graph(task_dir, repo_root)
    if metadata_errors:
        print(
            colored(
                f"Error: refusing to archive '{task_name}': invalid task metadata",
                Colors.RED,
            ),
            file=sys.stderr,
        )
    ...
    blocked_dependencies = incomplete_dependencies(task_dir, repo_root)
    if blocked_dependencies:
        print(
            colored(
                f"Error: refusing to archive '{task_name}': dependencies are not completed",
                Colors.RED,
            ),
            file=sys.stderr,
        )
    ...
    unchecked = unchecked_acceptance_items(task_dir / "prd.md")
    if unchecked:
        print(
            colored(
                f"Error: refusing to archive '{task_name}': acceptance criteria remain unchecked",
                Colors.RED,
            ),
            file=sys.stderr,
        )
        for line_number, item in unchecked:
            print(f"  - prd.md:{line_number}: {item}", file=sys.stderr)
        return 1
...
def _auto_commit_archive(
    ...
    Scoped narrowly to the archived task's source + exact destination paths
