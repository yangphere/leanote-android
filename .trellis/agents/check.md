...
1. **Get the complete working-tree scope** — inspect tracked staged/unstaged changes and every non-ignored untracked file
...
1. Run all of the following before scoping the review:
   - `git status --short --untracked-files=all`
   - `git diff --name-only` and `git diff`
   - `git diff --staged --name-only` and `git diff --staged`
   - `git ls-files --others --exclude-standard`
   Read every relevant untracked file directly because it has no Git diff yet.
