...
<!--
  ...
  Editing checklist:
    ...
    - Full runtime contract:
      .trellis/workflow.md (the workflow-state blocks below)
-->
...
Record executable ordering in `task.json.meta.depends_on` as a list of full task directory names. `task.py start` and `task.py archive` refuse while any reachable dependency is not completed. `task.py validate` checks these references, dependency cycles, reciprocal parent/child links, and basic status/date consistency.

Archive also refuses while `prd.md` contains unchecked items under an `Acceptance Criteria` or `验收` heading. For work intentionally performed directly on its target branch rather than through a PR branch, set `task.json.meta.delivery_mode` to `"direct"`; this is the only mode in which `branch == base_branch` is accepted without `--skip-branch-validation`.
...
- `.trellis/workflow.md` — workflow-state blocks, lifecycle rules, and customization contract
